...
Hot storage enables faster downloads by retrieving files from a hot storage node pre-selected by the router. If the file is not cached in hot storage, the router triggers a prefetch and the client falls back to the regular download path automatically.

Prerequisites:
- A hot storage router URL
- Your private key (used to sign download requests)
- A deposit made to the hot storage router (required for billing)
...
The `--private-key` flag accepts keys with or without the `0x` prefix. For fragment downloads, use `--roots` instead of `--root`. Encryption (`--encryption-key`) is also supported alongside hot storage.
